package com.example.module_audio.player;
...
public class PlaybackService extends Service implements IPlayback, IPlayback.Callback {
    ...
    private void showNotification(PlayState status) {
        ...
        PendingIntent contentIntent = PendingIntent.getActivity(this, 0, new Intent(this, AudioListActivity.class), 0);

        // Set the info for the views that show in the notification panel.
        Notification notification = new NotificationCompat.Builder(this)
                .setSmallIcon(R.mipmap.arms_component_logo)  // the status icon
                .setWhen(System.currentTimeMillis())  // the time stamp
                .setLargeIcon(BitmapFactory.decodeResource(this.getResources(),R.mipmap.arms_component_logo))
...
//        ARouter.getInstance().build(RouterHub.APP_MAINACTIVITY).navigation();
    ...
    private RemoteViews getSmallContentView() {
        if (mContentViewSmall == null) {
            mContentViewSmall = new RemoteViews(getPackageName(), R.layout.audio_remote_view_music_player_small);
    ...
    private RemoteViews getBigContentView() {
        if (mContentViewBig == null) {
            mContentViewBig = new RemoteViews(getPackageName(), R.layout.audio_remote_view_music_player);
    ...
    private void setUpRemoteView(RemoteViews remoteView) {
        remoteView.setImageViewResource(R.id.image_view_close, R.drawable.audio_ic_remote_view_close);
        remoteView.setImageViewResource(R.id.image_view_play_last, R.drawable.audio_ic_remote_view_play_last);
        remoteView.setImageViewResource(R.id.image_view_play_next, R.drawable.audio_ic_remote_view_play_next);
    ...
    private void updateRemoteViews(RemoteViews remoteView) {
        remoteView.setImageViewResource(R.id.image_view_play_toggle, isPlaying()
                ? R.drawable.audio_ic_remote_view_pause : R.drawable.audio_ic_remote_view_play);
